...
let cats = ['land', 'ia', 'island', 'stan'];
let categories = [[], [], [], []];
let categoriess = [];

// const categorizeCountries = (coun) => {
const don = (val) =>
  countriess.filter(({ name }) => name.toLowerCase().endsWith(val));
cats.forEach((c) => categoriess.push(don(c)));
...
function countryL(coun) {
  const arr = [];
  letters.forEach((l) => {
    coun.forEach(({ name }) => {
      if (name.toLowerCase().startsWith(l)) {
        const indx = arr.findIndex((a) => a.l === l);
        if (indx === -1) {
          arr.push({ l, count: 1 });
        } else {
          arr[indx].count += 1;
...
console.log(countryL(countriess));
...
function countryLetters() {
  ...
  countriess.forEach((country) => {
    const letterIndex = categories.findIndex((category) =>
      country.name.toLowerCase().startsWith(category.letter)
    );
...
const getFirst10 = (sortFunction) => {
  if (!sortFunction) return countriess.slice(0, 10);
  return sortFunction(countriess).slice(0, 10);
};
const getLast10 = (sortFunction) => {
  if (!sortFunction) return countriess.slice(countriess.length - 10);

  return sortFunction(countriess).slice(countriess.length - 10);
};

console.log(getFirst10());
console.log(getLast10());

console.log(
  getFirst10((kontriz) => kontriz.sort((a, b) => a.population - b.population))
);
console.log(
  getLast10((kontriz) => kontriz.sort((a, b) => a.population - b.population))
);
console.log(
  getFirst10((kontriz) =>
    kontriz.sort((a, b) => b.languages.length - a.languages.length)
  )
);
console.log(
  countriess.sort((a, b) => a.name - b.name).map((country) => country.name)
);
...
// let duplicates = numArray.filter(
//   (val, index) => index !== numArray.indexOf(val)
// );
